...
function parseMarkdown(text) {
  if (!text) return text;
  let cleaned = text.replace(/^\s*\[AI Insight\]\s*/i, '');
  ...
  return parts.map((part, pi) => {
    ...
    return lines.map((line, li) => {
      ...
      if (bulletMatch) {
        return <div key={`${pi}-${li}`} style={{ paddingLeft: 10, position: 'relative', marginBottom: 1 }}>
      ...
      if (numMatch) {
        return <div key={`${pi}-${li}`} style={{ paddingLeft: 14, position: 'relative', marginBottom: 1 }}>
          <span style={{ position: 'absolute', left: 0, color: C.t3, fontSize: 10, fontWeight: 600 }}>{numMatch[1]}.</span>
          {renderInline(numMatch[2])}
        </div>;
      }
      return <span key={`${pi}-${li}`}>{renderInline(line)}{li < lines.length - 1 ? '\n' : ''}</span>;
    });
  });
}
...
export default function MsgBubble({ msg, streaming }) {
  ...
  if (msg.tier === 'insight') {
    return (
      <div style={{
        ...
        fontSize: 12, lineHeight: 1.5, color: C.t1,
      }}>
  ...
  if (msg.tier === 'brief') {
    return (
      <div style={{
        ...
        fontSize: 12, lineHeight: 1.6, color: C.t1,
      }}>
        {parseMarkdown(msg.content)}
  ...
  return (
    <div style={{
      ...
      marginBottom: 8, animation: 'copilotInlineMsgIn 0.3s ease forwards',
    }}>
      <div style={{
        ...
        {streaming && (
          <span style={{
            ...
            background: C.b, marginLeft: 2, borderRadius: 1,
          ...
          <>
            <button
              ...
              style={{
                ...
                fontSize: 10, padding: '0 2px', opacity: 0.5, color: C.t3,
              }}
              title="Helpful"
            >👍</button>
            <button
