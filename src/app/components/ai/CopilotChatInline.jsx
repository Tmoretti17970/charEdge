...
export default function CopilotChatInline() {
  ...
  const [ctaDismissed, setCtaDismissed] = useState(() => {
    try { return localStorage.getItem('charEdge-model-cta-dismissed') === 'true'; } catch { return false; }
  });
  ...
  const handleCtaDismiss = useCallback(() => {
    ...
    try { localStorage.setItem('charEdge-model-cta-dismissed', 'true'); } catch { /* */ }
  }, []);
  ...
  const handleInputChange = useCallback((e) => {
    ...
    if (val.startsWith('/')) {
      import('../../../ai/SlashCommandParser').then(({ slashCommandParser }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', marginTop: -16, marginLeft: -16, marginRight: -16 }}>
      {/* Header */}
      <div style={{
          ...
          {searchMode === 'keyword' && (
            <span
              ...
              style={{
                ...
                color: '#f59e0b', cursor: 'default', whiteSpace: 'nowrap',
              }}
            >
              🔤 Keyword search
            </span>
          ...
          {import.meta.env.DEV && tokenReport && (
            <span
              ...
              style={{
                ...
                fontFamily: 'monospace', cursor: 'default', whiteSpace: 'nowrap',
              }}
            >
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {hasMessages && (
            <button
              ...
              onMouseEnter={(e) => { e.currentTarget.style.background = C.sf2; e.currentTarget.style.color = C.t1; }}
          ...
          {hasMessages && (
            <button
              ...
              onMouseEnter={(e) => { e.currentTarget.style.background = C.r + '12'; e.currentTarget.style.color = C.r; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = 'none'; e.currentTarget.style.color = C.t3; }}
            >
              🗑
            </button>
      ...
      {showClearConfirm && (
        <div style={{
          padding: '6px 16px', background: C.r + '10',
          borderBottom: `1px solid ${C.r}20`, borderTop: `1px solid ${C.r}20`,
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          fontSize: 11, color: C.t2, flexShrink: 0,
        }}>
          <span>Clear all history?</span>
          <div style={{ display: 'flex', gap: 4 }}>
            <button onClick={() => { clearAllHistory(); setShowClearConfirm(false); }}
              style={{ background: C.r + '20', border: `1px solid ${C.r}40`, borderRadius: 4, color: C.r, fontSize: 10, fontWeight: 600, padding: '2px 6px', cursor: 'pointer', fontFamily: F }}>
              Clear
            </button>
            <button onClick={() => setShowClearConfirm(false)}
              style={{ background: C.sf2, border: `1px solid ${C.bd}`, borderRadius: 4, color: C.t2, fontSize: 10, padding: '2px 6px', cursor: 'pointer', fontFamily: F }}>
              Cancel
            </button>
          </div>
        </div>
        ...
        {!hasMessages && !isStreaming ? (
          <div style={{ padding: '12px 0', fontSize: 12, color: C.t3, lineHeight: 1.6 }}>
            <p style={{ margin: '0 0 10px' }}>
              Ask about your trades, strategies, risk, or anything else.
            </p>
          </div>
        ) : (
          <>
            ...
            {isStreaming && streamingText && (
              <MsgBubble
                msg={{ id: '__streaming', role: 'assistant', content: streamingText, timestamp: Date.now(), tier: 'L3' }}
                streaming
              />
            )}
            ...
            {modelPromptShown && (
              <div style={{
                ...
                margin: '8px 0', animation: 'copilotInlineMsgIn 0.3s ease',
              }}>
                ...
                <div style={{ fontSize: 10, color: C.t2, lineHeight: 1.5, marginBottom: 8 }}>
                  Download SmolLM2 (~80MB) to run a real language model in your browser.
                  Your question will be automatically re-sent after download.
                </div>
                {modelDownloading ? (
                  <div>
                    <div style={{
                      height: 4, borderRadius: 4, background: C.sf2,
                    ...
                    <div style={{ fontSize: 9, color: C.t3 }}>
                      {modelProgressText || `${modelProgress}%`}
                    </div>
                  </div>
                ) : (
                  <div style={{ display: 'flex', gap: 6 }}>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        background: C.sf2, color: C.t2, border: `1px solid ${C.bd}`,
            ...
            {error && (
              <div style={{
                padding: '6px 10px', borderRadius: 8, background: C.r + '12',
                border: `1px solid ${C.r}25`, fontSize: 11, color: C.r,
                marginTop: 6, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6,
              }}>
                <span>⚠️ {error}</span>
                <button onClick={retryLast} style={{
                  background: C.r + '20', border: `1px solid ${C.r}40`, borderRadius: 4,
                  color: C.r, fontSize: 10, fontWeight: 600, padding: '2px 8px',
      ...
      <SlashAutocomplete
        ...
        selectedIdx={cmdSelectedIdx}
        onSelect={handleCmdSelect}
      />

      {/* Input */}
      <div style={{
        padding: '8px 16px 10px', borderTop: `1px solid ${C.bd}`,
        ...
        <textarea
          ...
          style={{
            ...
            background: C.sf2, color: C.t1,
            fontSize: 12, fontFamily: F,
            padding: '7px 10px', outline: 'none',
            lineHeight: 1.5, minHeight: 32, maxHeight: 96,
        ...
        {isStreaming ? (
          <button
            ...
            onMouseEnter={(e) => { e.currentTarget.style.background = C.r + '25'; }}
            onMouseLeave={(e) => { e.currentTarget.style.background = C.r + '15'; }}
          >
            ■
          </button>
        ) : (
          <button
            ...
            style={{
              ...
              background: input.trim()
                ? `linear-gradient(135deg, ${C.b}, ${C.bH})` : C.sf2,
