...
function TradeHeatmap({ symbol, priceMin, priceMax, _height = 400 }) {
  ...
  const [enabled, setEnabled] = useState(() => {
    try {
      return localStorage.getItem('tf-heatmap-enabled') === 'true';
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem('tf-heatmap-enabled', String(enabled));
    } catch {
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {profile?.totalEvents > 0 && (
            <span style={{ fontSize: 8, fontFamily: 'var(--tf-mono)', color: C.t3 }}>{profile.totalEvents} trades</span>
          )}
        </div>
      ...
      {bins.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
          {bins
            ...
            .map((bin, i) => {
              const entryW =
                showEntries && bin.entries > 0 ? (bin.entries / (bin.total || 1)) * bin.normalized * 100 : 0;
