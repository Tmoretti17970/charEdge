...
function AdvisorCard({ card }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 11, color: C.t3, fontFamily: F, lineHeight: 1.4 }}>{card.detail}</div>
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <div style={{ fontSize: 11, color: C.t3, fontFamily: F }}>{riskCheck.message}</div>
      </div>
    </div>
  );
}
...
function PropFirmAdvisorPanel({ open, onClose }) {
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <div style={{ fontSize: 15, fontWeight: 800, color: C.t1 }}>🧠 AI Advisor</div>
          {activeProfile && (
            <div style={{ fontSize: 10, color: C.t3, fontFamily: M, marginTop: 2 }}>{activeProfile.name}</div>
          )}
        </div>
        <button
          onClick={onClose}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '14px 16px' }}>
        {!activeProfile ? (
          ...
          >
            ...
            <br />
            Set up a profile to get AI-powered advice.
          </div>
        ) : !advice ? (
          <div style={{ color: C.t3, fontSize: 12, textAlign: 'center', padding: 20 }}>Loading advisor…</div>
        ) : (
          <>
            {/* Risk Badge */}
