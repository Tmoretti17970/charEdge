...
const TEMPLATES = [
  ...
  { name: 'Momentum',       icon: '🚀', rules: [{ field: 'changePercent', op: 'gt', value: 3 }, { field: 'rsi', op: 'gt', value: 55 }] },
...
function RuleRow({ rule, index, onChange, onRemove }) {
  ...
  return (
    <div style={{
      ...
      padding: '6px 8px', borderRadius: radii.md,
...
function SmartFolderManager({ open, onClose }) {
  ...
  // Preview: count matching items (simplified)
  const matchCount = useMemo(() => {
    if (rules.length === 0) return 0;
    return items.filter(() => true).length; // placeholder — real evaluation requires live ticker data
  }, [rules, items]);
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '14px 16px' }}>
        ...
        {rules.map((rule, i) => (
          <RuleRow key={i} rule={rule} index={i}
            onChange={handleChangeRule} onRemove={handleRemoveRule} />
        ))}
        ...
        {folders && folders.filter(f => f.smart).length > 0 && (
          <div style={{ marginTop: 16 }}>
            ...
            {folders.filter(f => f.smart).map(f => (
              <div key={f.id} style={{
