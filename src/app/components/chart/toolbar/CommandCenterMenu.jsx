...
function ToggleItem({ label, active, onClick }) {
  return (
    <button className="tf-command-item" data-active={active || undefined} onClick={onClick}>
...
function ActionItem({ label, onClick, accent }) {
  return (
    <button className="tf-command-item" data-accent={accent || undefined} onClick={onClick}>
      <span className="tf-command-item-label">{label}</span>
          ...
          >
            {lo.grid.map((row, ri) => (
              <div key={ri} className="tf-command-layout-row">
                {row.map((_, ci) => (
                  <div key={ci} className="tf-command-layout-cell" data-active={isActive || undefined} />
                ))}
              </div>
            ))}
          </button>
...
export default function CommandCenterMenu({
  ...
  const renderTrading = () => (
    <>
      ...
      />
      {!isMobile && <ToggleItem label="Show Trades" active={showTrades} onClick={() => setShowTrades(!showTrades)} />}
      <ToggleItem
  ...
  const renderTools = () => (
    <>
      ...
      />
      <ActionItem
        label={replayMode ? 'Exit Replay' : 'Bar Replay'}
        onClick={() => {
          toggleReplay();
          close();
        }}
    ...
    <div ref={menuRef} style={{ position: 'relative', flexShrink: 0 }}>
      ...
      {menuOpen && (
        <div className="tf-command-center" role="dialog" aria-label="Command Center">
          ...
          {/* ─── Tab Content ──────────────────────────── */}
          <div className="tf-command-content">{TAB_RENDERERS[activeTab]?.()}</div>
