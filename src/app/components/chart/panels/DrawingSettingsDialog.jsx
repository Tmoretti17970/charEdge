...
import {
  ...
  loadTemplates,
  deleteTemplate,
} from '../../../../charting_library/tools/engines/DrawingTemplates.js';
...
// Phase 5: Tool config extracted to separate module
import {
  getToolConfig,
  EXTEND_OPTIONS,
  STATS_OPTIONS,
  STATS_POSITION_OPTIONS,
  LABEL_POSITION_OPTIONS,
  DEFAULT_FIB_LEVELS,
  TIMEFRAME_ROWS,
  RISK_UNIT_OPTIONS,
  QTY_PRECISION_OPTIONS,
} from './drawingToolConfig.ts';
...
export default function DrawingSettingsDialog({ drawing, engine, onClose }) {
  ...
  const [showFill, _setShowFill] = useState(s.showFill !== false);
  ...
  const [visibilityTicks, setVisibilityTicks] = useState(drawing?.visibility?.ticks !== false);
  const [visibilityRanges, setVisibilityRanges] = useState(drawing?.visibility?.ranges !== false);
  ...
  const handleLoadTemplate = (tmpl) => {
    ...
    if (ts.color) {
      setColor(ts.color);
      updateStyle('color', ts.color);
    }
    if (ts.lineWidth) {
      setLineWidth(ts.lineWidth);
      updateStyle('lineWidth', ts.lineWidth);
    }
    if (ts.dash) {
      setDash(ts.dash);
      updateStyle('dash', ts.dash);
    }
    if (ts.fillColor) {
      setFillColor(ts.fillColor);
      updateStyle('fillColor', ts.fillColor);
    }
    if (ts.extend) {
      setExtend(ts.extend);
      updateStyle('extend', ts.extend);
    }
    if (ts.textColor) {
      setTextColor(ts.textColor);
      updateStyle('textColor', ts.textColor);
    }
    if (ts.fontSize) {
      setFontSize(ts.fontSize);
      updateStyle('fontSize', ts.fontSize);
    }
  };
    ...
    <div className={st.templateFooter}>
      ...
      {showTemplateSave ? (
        <div className={st.templateSaveRow}>
          ...
          />
          <button onClick={handleSaveTemplate} className={st.templateSaveBtn}>
            Save
          </button>
          <button onClick={() => setShowTemplateSave(false)} className={st.templateCancelBtn}>
            ✕
          </button>
        </div>
      ) : (
        <button onClick={() => setShowTemplateSave(true)} className={st.templateNewBtn}>
          + Save Template
        </button>
      )}
      {templates.length > 0 && (
  ...
  return (
    ...
    >
        ...
        <div>
          <div className={`${st.flexRowGap8} ${st.mb12}`}>
            <StepperInput
              ...
              min={0}
            />
            <SelectDropdown value="default" options={[{ id: 'default', label: 'Default' }]} onChange={() => {}} />
          </div>
          <StepperInput
            label="Lot size"
            value={lotSize}
            onChange={applyAndSet(setLotSize, 'lotSize')}
            step={0.01}
            min={0.01}
          />
          <div className={`${st.flexRowGap8} ${st.padY6}`}>
            ...
            <SelectDropdown
              value={riskUnit}
              options={RISK_UNIT_OPTIONS}
              onChange={applyAndSet(setRiskUnit, 'riskUnit')}
            />
          </div>
          <StepperInput
            label="Entry price"
            value={entryPrice}
            onChange={applyAndSet(setEntryPrice, 'entryPrice')}
            step={0.1}
          />
          <StepperInput
            label="Leverage"
            value={leverage}
            onChange={applyAndSet(setLeverage, 'leverage')}
            step={1}
            min={1}
          />
          ...
          <StepperInput
            label="Ticks"
            value={profitTicks}
            onChange={applyAndSet(setProfitTicks, 'profitTicks')}
            step={1}
            min={0}
          />
          <StepperInput
            label="Price"
            value={profitPrice}
            onChange={applyAndSet(setProfitPrice, 'profitPrice')}
            step={0.1}
          />
          ...
          <StepperInput
            label="Ticks"
            value={stopTicks}
            onChange={applyAndSet(setStopTicks, 'stopTicks')}
            step={1}
            min={0}
          />
          ...
          <div className={st.padY8}>
            <SelectDropdown
              label="QTY precision"
              value={qtyPrecision}
              options={QTY_PRECISION_OPTIONS}
              onChange={applyAndSet(setQtyPrecision, 'qtyPrecision')}
            />
          </div>
        </div>
      )}
        ...
        <div>
          ...
          {config.hasLineCompound && (
            <LineCompound
              ...
              onColorChange={
                config.hasBorder ? applyAndSet(setBorderColor, 'borderColor') : applyAndSet(setColor, 'color')
              }
              lineWidth={lineWidth}
          ...
          {config.hasBackground && (
            <div className={`${st.flexRowGap8} ${st.padY4}`}>
              ...
              />
              {showBackground && <ColorSwatch color={fillColor} onChange={applyAndSet(setFillColor, 'fillColor')} />}
            </div>
          )}
            ...
            <>
              ...
              <div className={st.fibGrid}>
                {fibLevels.map((level, i) => (
                  <div key={i} className={st.fibRow} data-visible={level.visible}>
                    <button
                      onClick={() => {
              ...
              <CheckboxRow
                ...
                onChange={(v) => {
                  setShowPercentages(v);
                  updateStyle('showPercentages', v);
                }}
              />
              <SelectDropdown
          ...
          <div className={st.mt16}>
            <button onClick={handleSaveAsDefault} className={st.saveDefaultBtn}>
      ...
      {activeTab === 'coordinates' && (
        <div>
          {points.map((pt, i) => (
            <div key={i} className={st.coordRow}>
              <span className={st.coordLabel}>#{i + 1} (price, bar)</span>
              <StepperInput
                value={pt.price}
          ...
          {points.length === 0 && <div className={st.coordEmpty}>No anchor points for this drawing type.</div>}
        </div>
      )}
