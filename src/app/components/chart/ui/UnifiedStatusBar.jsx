...
import { C, getAssetClass } from '@/constants.js';
...
function SpeedMeter({ rate, maxRate = 100 }) {
  ...
  return (
    <div className={s.s1}>
      {heights.map((h, i) => (
        <div key={i} className={s.speedPip} style={{ height: h, '--pip-color': i < filled ? color : undefined }} />
      ))}
    </div>
  );
}
  ...
  return (
    <div className="tf-chart-status-bar" id="unified-status-bar">
              ...
              <span className={s.s2}>
                ...
                {health.performance.fps > 0 && (
                  <>
                    <span className={s.emoji}>⚡</span>
                    <span
                      className={s.fpsValue}
                      style={{
                        '--dyn-color':
                          health.performance.fps < 15
                            ? '#ef4444'
                            : health.performance.fps < 30
                              ? '#f59e0b'
                              : 'var(--tf-t1)',
                      }}
                    >
                ...
                <span className={s.s8}>{health.performance.qualityLevel}</span>

                {/* Stale data */}
                {staleMs >= STALE_THRESHOLD && (
                  <>
                    <span className={s.s9}>⚠ Stale</span>
                    <span className={s.s10}>{formatElapsed(staleMs)}</span>
                  </>
                )}
              </span>
            )}
      ...
      <span className="tf-status-group">
        ...
        <span
          className={`tf-status-value ${s.ohlcValue}`}
          style={{ '--dyn-color': displayBar.open >= (prevBar?.close || 0) ? C.g : C.r }}
        >
      ...
      <span className="tf-status-group">
        ...
        <span className={`tf-status-value ${s.ohlcValue}`} style={{ '--dyn-color': C.g }}>
      ...
      <span className="tf-status-group">
        ...
        <span className={`tf-status-value ${s.ohlcValue}`} style={{ '--dyn-color': C.r }}>
          {formatPrice(displayBar.low)}
        </span>
      </span>
      <span className="tf-status-dot" />
      <span className={`tf-status-group tf-status-close${pulsing ? ' tf-price-pulse' : ''}`} ref={closeBadgeRef}>
      ...
      {/* ─── Market Status ─── */}
      {marketStatus && (
        <span className={`tf-status-group ${s.marketGroup}`} title={`${marketStatus} — ${getAssetClass(symbol)}`}>
          ...
          <span
            className={`tf-status-value ${s.marketValue}`}
            style={{
              '--dyn-color': marketStatus === 'Market Closed' ? C.t3 : marketStatus === 'Extended Hours' ? C.y : C.g,
            }}
          >
            {marketStatus === '24/7'
              ? '24/7'
      ...
      {dataAge != null && dataAge > 5 * 60_000 && (
        ...
        >
          ...
          <span className={`tf-status-value ${s.staleValue}`}>{Math.round(dataAge / 60_000)}m</span>
        </span>
      )}
      ...
      {/* ─── Error Badge ─── */}
      {expanded && hasErrors && (
        <span className={s.s11} onClick={toggleErrors} title="Click to view pipeline logs">
          ⚠ {health.errors.total + health.errors.warnings}
          {showErrors && (
            <div className={s.s12} onClick={(e) => e.stopPropagation()}>
              <div className={s.s13}>Pipeline Log</div>
              {recentErrors.length === 0 && <div className={s.s14}>No recent issues</div>}
              {recentErrors.map((entry, i) => (
                <div key={i} className={s.logEntry} data-level={entry.level}>
                  ...
                  <span className={s.logSource}>[{entry.source}]</span> {entry.message}
                  {entry.error && <div className={s.s15}>└ {entry.error}</div>}
                </div>
              ))}
            </div>
          )}
      ...
      {/* ─── Bar Countdown ─── */}
      {countdown && (
        <span className={`${isUrgent ? 'tf-countdown-urgent' : ''} ${s.s16}`} title="Time until next bar close">
      ...
      <div ref={tzRef} className={s.s17}>
        <span className={s.liveClock}>{liveClock}</span>
        <button
        ...
        {tzOpen && (
          <div className={`tf-chart-dropdown ${s.s19}`}>
      ...
      <div ref={scaleRef} className={s.scaleWrap}>
        ...
        {scaleOpen && (
          <div className={`tf-chart-dropdown ${s.s23}`}>
