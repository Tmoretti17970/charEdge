 ...
 */
const ChartDataTable = memo(function ChartDataTable({ bars = [], visible = false, symbol = '', maxRows = 100 }) {
  ...
  return (
    <div role="region" aria-label={`${symbol} price data table`} style={{ ...srOnlyStyle, ...visibleStyle }}>
      <table
        style={{
          width: '100%',
      ...
      >
        ...
        <tbody>
          {displayBars.map((bar, i) => {
            const change = bar.close && bar.open ? (((bar.close - bar.open) / bar.open) * 100).toFixed(2) : '0.00';
            ...
            return (
              ...
              >
                ...
                <td
                  style={{
                    ...tdStyle,
                    color: isUp ? 'var(--tf-green)' : 'var(--tf-red)',
                  }}
                >
                  {isUp ? '+' : ''}
                  {change}%
