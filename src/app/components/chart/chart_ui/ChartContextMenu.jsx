...
export default function ChartContextMenu({ menu, onClose, handlers, tradeMode, tradeStep }) {
  ...
  if (tradeMode) {
    ...
    if (tradeStep === 'entry' || tradeStep === 'idle') {
      items.push({
        label: (
          <>
            <Icon name="pin" size={11} /> Set Entry @ {price}
          </>
        ),
        action: () => handlers.onSetEntry?.(menu.price, menu.barIdx),
      });
    }
    if (tradeStep === 'sl' || tradeStep === 'entry') {
      items.push({
        label: (
          <>
            <Icon name="stop-loss" size={11} /> Set Stop Loss @ {price}
          </>
        ),
    ...
    if (tradeStep === 'tp' || tradeStep === 'sl') {
      items.push({
        label: (
          <>
            <Icon name="target" size={11} /> Set Target @ {price}
          </>
        ),
    ...
    items.push({
      label: (
        <>
          <Icon name="long" size={11} /> Long Entry @ {price}
        </>
      ),
    ...
    items.push({
      label: (
        <>
          <Icon name="short" size={11} /> Short Entry @ {price}
        </>
      ),
    ...
    items.push({
      label: (
        <>
          <Icon name="bell" size={11} /> Add Alert @ {price}
        </>
      ),
      action: () => handlers.onAddAlert?.(menu.price),
    });
    items.push({
      label: (
        <>
          <Icon name="edit" size={11} /> Quick Journal
        </>
      ),
      action: handlers.onQuickJournal,
    });
    items.push(SEPARATOR);
    items.push({
      label: (
        <>
          <Icon name="changelog" size={11} /> Copy Price
        </>
      ),
      action: () => handlers.onCopyPrice?.(menu.price),
    });
  }
  ...
  return (
    ...
    >
      <div className={s.priceHeader} style={{ color: C.t3, fontFamily: M, borderBottom: `1px solid ${C.bd}` }}>
        Price: {price}
        {menu.date && <span className={s.priceDate}>{new Date(menu.date).toLocaleDateString()}</span>}
      </div>
