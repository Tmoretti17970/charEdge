...
function ToolTip({ text, shortcut, pos }) {
  ...
  return (
    <div
      ...
      style={{ position: 'fixed', top: pos.top, left: pos.left + 8, transform: 'translateY(-50%)' }}
    >
      {text}
...
export default function DrawingSidebar({ isOpen, onClose }) {
  ...
  const onDragStart = (e) => {
    ...
    const onUp = () => {
      ...
      setPos((p) => {
        try {
          localStorage.setItem('charEdge-draw-pos', JSON.stringify(p));
        } catch {
          /* no-op */
        }
        return p;
  ...
  const handleClearAll = () => {
    ...
    if (st.drawings.length > 0) {
      useChartToolsStore.setState({
        drawingHistory: [...st.drawingHistory.slice(-49), st.drawings],
        drawingFuture: [],
      });
      st.setDrawings([]);
    }
    ...
    <>
      ...
      >
        <div className={s.dragHandle} onMouseDown={onDragStart}>
          ...
          <button onClick={onClose} className={s.gripClose} title="Hide palette">
            ✕
          </button>
        </div>
        ...
        <div className={s.utilRow}>
          ...
          >
            <svg width="12" height="12" viewBox="0 0 14 14" fill="none">
              <path
                d="M3 4h8l-.75 8.25a1 1 0 0 1-1 .75H4.75a1 1 0 0 1-1-.75L3 4z"
                stroke="currentColor"
                strokeWidth="1.1"
                fill="none"
              />
          ...
          {DRAWING_GROUPS.map((group) => {
            ...
            return (
              <button
                ...
                data-expanded={expandedGroup === group.id || undefined}
