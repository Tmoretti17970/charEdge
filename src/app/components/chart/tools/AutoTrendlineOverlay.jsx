...
export default function AutoTrendlineOverlay({ bars, priceToY, idxToX, chartWidth: _chartWidth, onAccept }) {
  ...
  const visibleLines = useMemo(
    () => trendlines.filter((l) => !dismissedTrendlines.has(l.id)),
            ...
            {isHovered && (
              <foreignObject x={midX - 70} y={midY - 18} width={140} height={28} style={{ pointerEvents: 'auto' }}>
                <div className={s.hoverLabel} style={{ borderColor: `${lineColor}30` }}>
                  <span className={s.aiBadge} style={{ color: lineColor, background: `${lineColor}18` }}>
                    AI
                  </span>
                  ...
                  <button onClick={() => handleDismiss(line)} className={s.dismissBtn}>
                    ✕
                  </button>
                </div>
              </foreignObject>
