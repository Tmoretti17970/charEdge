  ...
  return (
    <div className={s.wrapper}>
      <button onClick={() => setOpen(!open)} className={`tf-btn ${s.trigger}`}
        style={{ border: `1px solid ${C.bd}`, background: open ? alpha(C.b, 0.08) : 'transparent', color: open ? C.b : C.t3, fontFamily: 'var(--tf-font)' }}>
      ...
      {open && (
        <div className={s.dropdown} style={{ background: C.bg2, border: `1px solid ${C.bd}`, boxShadow: `0 8px 24px ${alpha('#000', 0.3)}` }}>
          <div className={s.dropdownTitle} style={{ color: C.t1, fontFamily: 'var(--tf-font)' }}>Layout Presets</div>
          ...
          <div className={s.presetList}>
            {Object.entries(PRESETS).map(([id, preset]) => (
              <button key={id} onClick={() => applyDiscoverPreset(id)} className={`tf-btn ${s.presetBtn}`}
                ...
                <div className={s.presetName} style={{ color: discoverPreset === id ? C.b : C.t1, fontFamily: 'var(--tf-font)' }}>{preset.label}</div>
                <div className={s.presetDesc} style={{ color: C.t3, fontFamily: 'var(--tf-font)' }}>{preset.description}</div>
              </button>
            ))}
          </div>

          <button onClick={() => { resetDiscoverLayout(); setOpen(false); }} className={`tf-btn ${s.resetBtn}`}
            style={{ border: `1px solid ${C.bd}`, color: C.t3, fontFamily: 'var(--tf-font)' }}>
            ↻ Reset to Default
          </button>
        </div>
      )}
