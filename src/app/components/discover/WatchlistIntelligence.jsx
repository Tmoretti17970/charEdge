            ...
            <div className={st.list}>
              {intel.map((item) => {
                ...
                return (
                  <div
                    key={item.symbol}
                    onClick={() => setExpandedSym(isExpanded ? null : item.symbol)}
                    className={st.intelRow}
                    style={{
                      background: alpha(C.sf, 0.5),
                      border: `1px solid ${alpha(C.bd, item.priority > 50 ? 0.8 : 0.4)}`,
                      borderLeft: `3px solid ${item.priority > 50 ? C.b : 'transparent'}`,
                    }}
                  >
                    <div className={st.intelMain}>
                      <div className={st.symCol}>
                        <div className={st.symName}>{item.symbol}</div>
                        <div className={st.symPrice} style={{ color: C.t2 }}>
                          ${item.price ? item.price.toFixed(2) : '—'}
                        </div>
                      </div>
                      <div className={st.changeCol}>
                        <span
                          className={st.changeBadge}
                          style={{
                            color: item.changePercent >= 0 ? C.g : C.r,
                            background: alpha(item.changePercent >= 0 ? C.g : C.r, 0.08),
                          }}
                        >
                          ...
                          return (
                            <div
                              ...
                              style={{
                                ...
                                background: alpha(item.changePercent >= 0 ? C.g : C.r, 0.5),
                              }}
                            />
                          );
                        })}
                      ...
                      <div className={st.patternCol} style={{ color: C.t2 }}>
                        {item.pattern}
                      </div>
                      <span
                        className={st.rowChevron}
                        style={{ color: C.t3, transform: isExpanded ? 'rotate(180deg)' : 'rotate(0deg)' }}
                      >
                        ▾
                      </span>
                    </div>
                      ...
                      <div className={st.detail} style={{ borderTop: `1px solid ${C.bd}` }}>
                        ...
                        <div>
                          ...
                          <div className={st.detailCol}>
                            ...
                            <span
                              className={st.volBadge}
                              style={{
                                color:
                                  item.volatilityRank === 'high' ? C.r : item.volatilityRank === 'low' ? C.g : C.t3,
                                background: alpha(
                                  item.volatilityRank === 'high' ? C.r : item.volatilityRank === 'low' ? C.g : C.t3,
                                  0.1,
                                ),
                            ...
                            <span className={st.confText} style={{ color: C.t3 }}>
