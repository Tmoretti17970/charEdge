    ...
    <div className={st.card} style={{ background: C.bg2, border: `1px solid ${C.bd}` }}>
      <button onClick={() => setCollapsed(!collapsed)} className={`tf-btn ${st.headerBtn}`}>
        <div className={st.headerLeft}>
          ...
          <LabsBadge />
          <span className={st.badge} style={{ color: C.t3, background: alpha(C.t3, 0.1) }}>{results.length} results</span>
        </div>
        ...
        <div className={st.body}>
          ...
          <div className={st.presetRow}>
            {SCAN_PRESETS.map((preset) => {
              ...
              return (
                <button key={preset.id} onClick={() => handlePresetClick(preset.id)} className={`tf-btn ${st.presetBtn}`} title={preset.description}
                  style={{ border: `1px solid ${isActive ? preset.color : C.bd}`, background: isActive ? alpha(preset.color, 0.12) : 'transparent', color: isActive ? preset.color : C.t2 }}>
          ...
          <div className={st.filterRow}>
            <div className={st.filterGroup}>
              {ASSET_CLASSES.map((ac) => {
                ...
                return (
                  <button key={ac.id} onClick={() => setAssetClass(ac.id)} className={`tf-btn ${st.filterBtn}`}
                    style={{ border: `1px solid ${isActive ? C.b : 'transparent'}`, background: isActive ? alpha(C.b, 0.08) : 'transparent', color: isActive ? C.b : C.t3 }}>
                    {ac.label}
                  </button>
                );
              })}
            ...
            <div className={st.sortGroup}>
              <span className={st.sortLabel} style={{ color: C.t3 }}>Sort:</span>
              {SORT_OPTIONS.map((opt) => {
                const isActive = sortBy === opt.id;
                return (
                  <button key={opt.id} onClick={() => handleSort(opt.id)} className={`tf-btn ${st.sortBtn}`}
                    style={{ background: isActive ? alpha(C.b, 0.1) : 'transparent', color: isActive ? C.b : C.t3, fontWeight: isActive ? 700 : 500 }}>
                    {opt.label}
          ...
          {/* Results */}
          {loading ? (
            <div className={st.loading} style={{ color: C.t3 }}>Scanning markets...</div>
          ) : results.length === 0 ? (
            <div className={st.emptyState} style={{ color: C.t3 }}>
              No results match this scan.{' '}
              <button onClick={() => { setActivePreset(null); setAssetClass('all'); }} className={`tf-btn ${st.resetBtn}`} style={{ color: C.b }}>Reset filters</button>
            </div>
          ) : (
            <div className={st.tableWrap}>
              ...
              {results.map((item) => <ScreenerRow key={item.symbol} item={item} />)}
            </div>
          )}
        </div>
      )}
...
function ScreenerRow({ item }) {
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      className={st.row}
      style={{ background: hovered ? alpha(C.b, 0.04) : 'transparent', border: `1px solid ${hovered ? alpha(C.b, 0.1) : 'transparent'}` }}>
      <div>
        <div className={st.symNameRow}>
          <span className={st.symText}>{item.symbol}</span>
          <span className={st.classBadge} style={{ color: C.t3, background: alpha(C.t3, 0.1) }}>{item.assetClass}</span>
        </div>
        <div className={st.subText} style={{ color: C.t3 }}>{item.name}</div>
      </div>
      <div className={st.priceText}>{formatPrice(item.price)}</div>
      <div className={st.changeText} style={{ color: isUp ? C.g : C.r }}>{isUp ? '+' : ''}{item.change.toFixed(1)}%</div>
      <div className={st.colRight}>
        <span className={st.rsiText} style={{ color: item.rsi > 70 ? C.r : item.rsi < 30 ? C.g : C.t2 }}>{item.rsi}</span>
        <RSIBar value={item.rsi} />
      </div>
      <div className={st.colRight}>
        <span className={st.volText} style={{ color: parseFloat(item.volumeRatio) > 1.5 ? C.b : C.t2 }}>{item.volumeRatio}x</span>
      </div>
      <div>
        <span className={st.signalBadge} style={{ color: item.signal.color, background: alpha(item.signal.color, 0.12) }}>{item.signal.label}</span>
      </div>
