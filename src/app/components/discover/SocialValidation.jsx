...
const MOCK_SYMBOLS = [
  {
    ...
    bearVotes: 22,
    conviction: 89,
    topIdea: 'AI capex cycle accelerating — $1000 by June',
    ideaAuthor: 'CryptoKing',
    ideaReturn: '+12.4%',
  },
  {
    symbol: 'TSLA',
    ...
    bearVotes: 45,
    conviction: 52,
    topIdea: 'Model Q launch = TAM expansion to $30K segment',
    ideaAuthor: 'SwingMaster',
    ideaReturn: '+4.8%',
  },
  {
    symbol: 'SPY',
    ...
    bearVotes: 52,
    conviction: 45,
    topIdea: 'Fed pivot delayed — range-bound until March FOMC',
    ideaAuthor: 'MacroAlpha',
    ideaReturn: '-1.2%',
  },
  {
    symbol: 'BTC',
    ...
    bearVotes: 18,
    conviction: 91,
    topIdea: 'ETF inflows + halving cycle = $85K target',
    ideaAuthor: 'NightOwl',
    ideaReturn: '+22.1%',
  },
  {
    ...
    bearVotes: 28,
    conviction: 78,
    topIdea: 'Ad revenue reacceleration + AI integration moat',
    ideaAuthor: 'ValueHunter',
    ideaReturn: '+8.6%',
  },
  {
    symbol: 'AAPL',
    ...
    bearVotes: 58,
    conviction: 38,
    topIdea: 'EU fines + China slowdown = headwinds',
    ideaAuthor: 'BearishBob',
    ideaReturn: '-3.4%',
  },
];
        ...
        <div className={st.body}>
          <div className={st.tabRow}>
            {[
              ...
              { id: 'ideas', label: '🏆 Top Ideas This Week' },
            ].map((t) => (
              <button
                key={t.id}
                onClick={() => setTab(t.id)}
                className={`tf-btn ${st.tabBtn}`}
                style={{
          ...
          {tab === 'consensus' ? (
            <div className={st.list}>
              {MOCK_SYMBOLS.map((sym) => {
                ...
                return (
                  <div
                    key={sym.symbol}
                    className={st.consRow}
                    style={{ background: alpha(C.sf, 0.5), border: `1px solid ${alpha(C.bd, 0.4)}` }}
                  >
                    <div className={st.consMain}>
                      <div className={st.symCol}>
                        <div className={st.symName}>{sym.symbol}</div>
                        <div className={st.watchCount} style={{ color: C.t3 }}>
                          👀 {sym.watchers.toLocaleString()}
                        </div>
                      </div>
                      <div className={st.voteWrap}>
                        ...
                        <div className={st.voteLegend}>
                          <span className={st.voteLabel} style={{ color: C.g }}>
                            🐂 {sym.bullVotes}%
                          </span>
                          <span className={st.voteLabel} style={{ color: C.r }}>
                            🐻 {sym.bearVotes}%
                          </span>
                        </div>
                      </div>
                      <div className={st.convCol}>
                        <div className={st.convValue} style={{ color: convColor }}>
                          {sym.conviction}
                        </div>
                        <div className={st.convLabel} style={{ color: C.t3 }}>
                          Conviction
                        </div>
                      </div>
                    </div>
                    <div className={st.ideaRow} style={{ background: alpha(C.sf, 0.3) }}>
                      <span className={st.ideaIcon} style={{ color: C.t3 }}>
                        💡
                      </span>
                      <div className={st.ideaText} style={{ color: C.t2 }}>
                        {sym.topIdea}
                      </div>
                      <span className={st.ideaAuthor} style={{ color: C.t3 }}>
                        @{sym.ideaAuthor}
                      </span>
                      <span className={st.ideaReturn} style={{ color: sym.ideaReturn.startsWith('+') ? C.g : C.r }}>
                        {sym.ideaReturn}
                      </span>
                    </div>
                  </div>
            ...
            <div className={st.list}>
              {TOP_IDEAS.map((idea) => (
                <div
                  key={idea.rank}
                  className={st.rankRow}
                  style={{
                    background: alpha(C.sf, idea.rank === 1 ? 0.7 : 0.4),
                    border: `1px solid ${idea.rank === 1 ? alpha(C.y, 0.2) : alpha(C.bd, 0.3)}`,
                  }}
                >
                  <span
                    className={st.rankNum}
                    style={{ color: idea.rank === 1 ? C.y : idea.rank === 2 ? '#c0c0c0' : '#cd7f32' }}
                  >
                    #{idea.rank}
                  </span>
                  <div className={st.rankBody}>
                    <div className={st.rankTitle}>{idea.title}</div>
                    <div className={st.rankMeta} style={{ color: C.t3 }}>
                      @{idea.author} · ${idea.symbol}
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div className={st.rankReturn} style={{ color: idea.return.startsWith('+') ? C.g : C.r }}>
                      {idea.return}
                    </div>
                    <div className={st.rankVotes} style={{ color: C.t3 }}>
                      👍 {idea.votes}
                    </div>
                  </div>
                </div>
