...
const GRADE_COLORS = {
  A: '#34c759', B: '#6e5ce6', C: '#f0b64e', D: '#ff9f0a', F: '#ff453a',
};
  ...
  if (!analysis || analysis.suggestions.length === 0) {
    return (
      <div style={{
        ...
        background: `${C.bd}06`, borderRadius: radii.md,
        border: `1px solid ${C.bd}15`,
        fontSize: 11, fontFamily: M, color: C.t3, fontStyle: 'italic',
      }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{
            ...
            background: `${gradeColor}15`, fontWeight: 900, fontSize: 12,
            fontFamily: M, color: gradeColor,
      ...
      {!collapsed && (
        <div style={{ padding: '0 16px 14px' }}>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 10 }}>
            {strengths.length > 0 && (
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 8, fontWeight: 700, fontFamily: M, color: '#34c759', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>
                  ✅ Strengths
                </div>
                {strengths.map((s, i) => (
                  <div key={i} style={{ fontSize: 9, fontFamily: M, color: C.t2, marginBottom: 2 }}>• {s}</div>
                ))}
              </div>
            )}
            {weaknesses.length > 0 && (
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 8, fontWeight: 700, fontFamily: M, color: '#ff453a', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>
                  ⚠️ Weaknesses
                </div>
                {weaknesses.map((w, i) => (
                  <div key={i} style={{ fontSize: 9, fontFamily: M, color: C.t2, marginBottom: 2 }}>• {w}</div>
                ))}
              </div>
            )}
          </div>
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                    ...
                    <span style={{
                      fontSize: 7, fontWeight: 800, fontFamily: M,
                  ...
                  {sg.action && (
                    <button
                      style={{
                        ...
                        borderRadius: radii.xs, fontSize: 9, fontWeight: 700, fontFamily: M,
                        background: `${imp.color}10`, color: imp.color,
