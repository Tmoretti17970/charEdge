...
function conditionColor(label) {
  if (label.includes('Up') || label.includes('🚀'))
    return { bg: alpha('#34d399', 0.12), border: alpha('#34d399', 0.25), text: '#34d399' };
  if (label.includes('Down') || label.includes('🔻'))
    return { bg: alpha('#f87171', 0.12), border: alpha('#f87171', 0.25), text: '#f87171' };
  if (label.includes('Reversal') || label.includes('⚠️'))
    return { bg: alpha('#fb923c', 0.12), border: alpha('#fb923c', 0.25), text: '#fb923c' };
...
function AIAnalysisPanel({ isOpen, onClose }) {
  ...
  const onDragStart = useCallback(
    (e) => {
      ...
      const onUp = () => {
        ...
        setPos((p) => {
          try {
            localStorage.setItem('charEdge-ai-panel-pos', JSON.stringify(p));
          } catch {
            /* no-op */
          }
          return p;
  ...
  const handleCopilotSubmit = useCallback((cmd) => {
    ...
    setTimeout(() => {
      try {
        ...
        setCopilotFeedback(
          result?.success ? { type: 'success', text: result.message } : { type: 'info', text: `"${cmd.trim()}" sent` },
        );
      } catch {
        setCopilotFeedback({ type: 'info', text: `"${cmd.trim()}" sent` });
      }
  ...
  return (
    <div className={`tf-ai-panel ${st.root}`} style={{ left: pos.x, top: pos.y }}>
            ...
            <div key={i} className={st.insightLine}>
              {line.split(/(\*\*.*?\*\*)/).map((seg, j) =>
                seg.startsWith('**') && seg.endsWith('**') ? (
                  <strong key={j} className={st.insightBold}>
                    {seg.slice(2, -2)}
                  </strong>
                ) : (
                  seg
