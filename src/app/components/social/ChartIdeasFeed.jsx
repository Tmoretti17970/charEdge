...
function EmptyFeedState({ zenMode, onPostIdea }) {
  return (
    ...
    >
      <div className="tf-empty-float" style={{ fontSize: 48, marginBottom: 16 }}>
        {zenMode ? '🔭' : '💡'}
      </div>
      <h3
        style={{
    ...
    >
      ...
      >
        ...
        <span
          ...
          style={{ display: 'flex', alignItems: 'center', gap: 5, cursor: 'pointer', transition: 'color 0.15s' }}
        ...
        <span
          onClick={handleShare}
          style={{ display: 'flex', alignItems: 'center', gap: 4, cursor: 'pointer', transition: 'color 0.15s', marginLeft: 'auto' }}
...
export default function ChartIdeasFeed({ zenMode = false, onPostIdea }) {
  const {
    feed,
    feedLoading,
    feedSort,
