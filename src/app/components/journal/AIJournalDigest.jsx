...
export function AutoTagRow({ trade, features }) {
  const result = useMemo(() => journalIntelligence.autoTag(trade, features || {}), [trade, features]);

  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 4 }}>
      <TradeTagPill label={`Grade ${result.quality}`} color={TAG_COLORS[result.quality]} />
      {result.tags.map((tag, i) => (
        <TradeTagPill key={i} label={tag} color={TAG_COLORS[result.setupType] || C.t3} />
      ))}
      {result.emotion !== 'neutral' && (
        <TradeTagPill label={result.emotion.toUpperCase()} color={result.emotion === 'fomo' ? C.r : C.y} />
      )}
    </div>
  );
}
  ...
  return (
    ...
    >
      <div>
        ...
        {trade.date && (
          <span style={{ color: C.t3, marginLeft: 6, fontSize: 10 }}>{new Date(trade.date).toLocaleDateString()}</span>
        )}
      </div>
      <div
        style={{
    ...
    <div style={{ marginBottom: 14 }}>
      ...
      >
        {[
          ...
          {
            label: 'WIN RATE',
            value: `${(digest.winRate * 100).toFixed(0)}%`,
            color: digest.winRate >= 0.5 ? C.g : C.r,
          },
          {
            label: 'NET P&L',
            value: `$${(digest.netPnl || 0).toFixed(0)}`,
            color: (digest.netPnl || 0) >= 0 ? C.g : C.r,
          },
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ color: C.t3, fontSize: 12, transition: transition.fast }}>{collapsed ? '▸' : '▾'}</span>
      </button>
            ...
            >
              ...
              >
                <span style={{ fontSize: 12, fontWeight: 700, color: C.t1, fontFamily: F }}>
                  {trade.symbol}
                  <span style={{ fontWeight: 400, color: C.t3, marginLeft: 6, fontSize: 10 }}>{trade.side}</span>
                </span>
