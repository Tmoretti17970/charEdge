...
function CommandPalette() {
  ...
  const actions = useMemo(
    () => ({
      ...
      addTrade: () => {
        setOpen(false);
        window.dispatchEvent(new CustomEvent('charEdge:global-quick-add'));
      },
      importCSV: () => {
        setPage('journal');
        setOpen(false);
        window.dispatchEvent(new CustomEvent('charEdge:import-csv'));
      },
      exportCSV: () => {
      ...
      setChartType: (type) => {
        useChartCoreStore.getState().setChartType(type);
        setPage('charts');
        setOpen(false);
      },
  ...
  if (open && mode === 'logbook') {
    return <SpotlightLogbook isOpen={true} onClose={() => setOpen(false)} filterDate={filterDate} />;
  }
      ...
      >
        ...
        >
          ...
          {query && (
            <button
              onClick={() => setQuery('')}
              style={{ background: 'none', border: 'none', color: C.t3, cursor: 'pointer', fontSize: 14, padding: 0 }}
            >
              ✕
            </button>
          )}
          {/* Mode pills */}
          <button style={pillActive} onClick={() => setMode('commands')}>
            ⌘ Commands
          </button>
          <button
            style={pillInactive}
              ...
              {cmds.map((cmd) => {
                ...
                return (
                  <CommandItem
                    ...
                    onSelect={() => setSelectedIdx(thisIdx)}
