      ...
      {expanded && (
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 16, fontSize: 10, color: C.t3, fontFamily: M }}>
            <span>Cost Basis: <strong style={{ color: C.t2 }}>${position.costBasis?.toFixed(2) || '—'}</strong></span>
            <span>Realized P&L: <strong style={{ color: position.realizedPnl >= 0 ? C.g : C.r }}>${position.realizedPnl?.toFixed(2) || '0.00'}</strong></span>
            <span>Fills: <strong style={{ color: C.t2 }}>{position.fills?.length || 0}</strong></span>
          </div>
          {position.fills && position.fills.length > 0 && (
            <div style={{ marginTop: 6, maxHeight: 100, overflowY: 'auto' }}>
              {position.fills.slice(-5).map((fill, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 9, color: C.t3, padding: '2px 0', fontFamily: M }}>
      ...
      <Card style={{ overflow: 'hidden', background: GLASS.subtle }}>
        ...
        <div style={{
          ...
          <span style={{ color: totalUnrealized >= 0 ? C.g : C.r, fontWeight: 700 }}>
            Total: {totalUnrealized >= 0 ? '+' : ''}{totalUnrealized.toFixed(2)}
          </span>
        </div>
      </Card>
    </div>
