  ...
  if (!summary || summary.tradeCount === 0) {
    return (
      ...
      >
        ...
        <div style={{ fontSize: 14, fontWeight: 600, color: C.t2, fontFamily: 'var(--tf-font)' }}>
          No trades this week
        </div>
        <div style={{ fontSize: 12, color: C.t3, fontFamily: 'var(--tf-mono)', marginTop: 4 }}>
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
        ...
        <div>
          <div style={{ fontSize: 14, fontWeight: 700, color: C.t1, fontFamily: 'var(--tf-font)' }}>Weekly Summary</div>
          <div style={{ fontSize: 11, color: C.t3, fontFamily: 'var(--tf-mono)' }}>Week of {summary.weekOf}</div>
        </div>
      </div>
      ...
      >
        ...
        <StatCard
          ...
          color={summary.winRate >= 50 ? '#00E676' : '#EF5350'}
        />
      </div>
      ...
      >
        <div style={{ fontSize: 12, color: C.t2, fontFamily: 'var(--tf-mono)', lineHeight: 1.7 }}>
          {summary.narrative}
        </div>
      </div>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {summary.topSymbols.map((s) => (
              ...
              >
                <span style={{ fontSize: 11, fontWeight: 700, color: C.t1, fontFamily: 'var(--tf-font)' }}>
                  {s.symbol}
                </span>
                <span
                  style={{
                ...
                <span style={{ fontSize: 9, color: C.t3, fontFamily: 'var(--tf-mono)' }}>×{s.count}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {summary.emotionBreakdown.best && (
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {summary.emotionBreakdown.best && (
              <EmotionChip
                label={`Best: "${summary.emotionBreakdown.best.emotion}"`}
                pnl={summary.emotionBreakdown.best.pnl}
                positive
              />
            )}
            {summary.emotionBreakdown.worst && (
              <EmotionChip
                label={`Worst: "${summary.emotionBreakdown.worst.emotion}"`}
                pnl={summary.emotionBreakdown.worst.pnl}
              />
            )}
          </div>
        </div>
...
function StatCard({ label, value, color }) {
  return (
    ...
    >
      ...
      <div style={{ fontSize: 16, fontWeight: 800, color: color || C.t1, fontFamily: 'var(--tf-font)', marginTop: 2 }}>
        {value}
      </div>
    </div>
  );
...
function EmotionChip({ label, pnl, positive }) {
  ...
  return (
    ...
    >
      <div style={{ fontSize: 11, fontWeight: 600, color: C.t2, fontFamily: 'var(--tf-font)' }}>{label}</div>
      <div style={{ fontSize: 13, fontWeight: 700, color, fontFamily: 'var(--tf-mono)', marginTop: 2 }}>
        {fmtUSD(pnl)}
      </div>
    </div>
  );
