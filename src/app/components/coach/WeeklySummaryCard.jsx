...
import st from './WeeklySummaryCard.module.css';
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {summary.topSymbols.map(s => (
              <div key={s.symbol} style={{
                ...
                <span style={{ fontSize: 11, fontWeight: 700, color: C.t1, fontFamily: 'var(--tf-font)' }}>{s.symbol}</span>
                <span style={{
        ...
        <div>
          ...
          {summary.keyMoments.map((m, i) => (
            <div key={i} style={{
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: C.t1, fontFamily: 'var(--tf-font)' }}>{m.description}</div>
...
function StatCard({ label, value, color }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 16, fontWeight: 800, color: color || C.t1, fontFamily: 'var(--tf-font)', marginTop: 2 }}>{value}</div>
    </div>
  );
}
...
function EmotionChip({ label, pnl, positive }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 13, fontWeight: 700, color, fontFamily: 'var(--tf-mono)', marginTop: 2 }}>{fmtUSD(pnl)}</div>
    </div>
  );
}
