  ...
  return (
    <div className={st.strengthWrap}>
      <div className={st.strengthRow}>
        {[1, 2, 3, 4, 5].map((i) => (
          <div key={i} className={st.strengthBar}
            style={{ background: i <= score ? color : `color-mix(in srgb, ${C.bd} 19%, transparent)` }} />
        ))}
      </div>
      <div className={st.strengthLabel} style={{ color }}>{label}</div>
    </div>
  );
}
...
function SecuritySection() {
  ...
  const handlePasswordChange = useCallback(async () => {
    ...
    try {
      ...
      setPasswordForm({ current: '', newPass: '', confirm: '' }); setShowPasswordForm(false);
  ...
  const handleSendReset = useCallback(async () => {
    ...
    setLoading(true);
    try { await resetPassword(resetEmail); toast.success('Password reset link sent — check your email'); setResetEmail(''); setShowResetForm(false); }
    catch (err) { toast.error(err.message || 'Failed to send reset email'); }
    finally { setLoading(false); }
  }, [resetEmail, resetPassword]);

  const handleSignOut = useCallback(async () => {
    try { await signOut(); toast.info('Signed out successfully'); }
    catch (err) { toast.error(err.message || 'Failed to sign out'); }
  }, [signOut]);
      ...
      <Card className={st.cardPad}>
        {isAuthenticated && !isLocal ? (
            ...
            <div className={st.bannerRow}>
              <div className={st.bannerLeft}>
                <div className={st.bannerIcon}
                  style={{ background: `linear-gradient(135deg, ${C.g}20, ${C.g}08)` }}>☁️</div>
                <div>
              ...
              <Btn variant="ghost" onClick={handleSignOut}
                style={{ fontSize: 11, padding: '4px 12px', color: C.r }}>Sign Out</Btn>
            </div>
          ...
          <div className={st.localRow}>
            <div className={st.bannerIcon}
              style={{ background: `linear-gradient(135deg, ${C.b}15, ${C.b}05)` }}>💻</div>
            <div className={st.localBody}>
              <div className={st.bannerTitle}>Running Locally</div>
              <div className={st.bannerSubFont}>Your data stays on this device. Set up cloud sync to enable email & password.</div>
            </div>
          </div>
        )}
      </Card>
          ...
          <div className={st.formCol}>
            <div>
              ...
              <input type="password" value={passwordForm.current}
                ...
                placeholder="Enter current password" style={{ ...inputStyle, width: '100%' }} />
            </div>
            <div>
              ...
              <input type="password" value={passwordForm.newPass}
                ...
                placeholder="Min. 8 characters" style={{ ...inputStyle, width: '100%' }} />
            ...
            <div>
              ...
              <input type="password" value={passwordForm.confirm}
                ...
                style={{ ...inputStyle, width: '100%', borderColor: passwordForm.confirm && !passwordsMatch ? C.r : undefined }} />
              {passwordForm.confirm && !passwordsMatch && <div className={st.formError}>Passwords don't match</div>}
            </div>
            <Btn onClick={handlePasswordChange} disabled={!passwordValid || loading}
      ...
      <Card className={st.cardPadLast}>
        ...
        {showResetForm && (
          <div className={st.resetRow}>
            ...
            <Btn onClick={handleSendReset} disabled={!resetEmail || loading}
