...
function StorageDashboard() {
  ...
  const fetchStorage = useCallback(async () => {
    ...
    try {
      ...
      setStorageData({ total: 0, quota: 50 * 1024 * 1024, categories: [{ name: 'Unknown', size: 0, color: CATEGORY_COLORS[0] }] });
    }
    setLoading(false);
  }, []);
  ...
  useEffect(() => {
    ...
    storageData.categories.forEach((cat) => {
      ...
      ctx.arc(cx, cy, inner, startAngle + slice, startAngle, true); ctx.closePath();
      ctx.fillStyle = cat.color; ctx.fill(); startAngle += slice;
    });
    ctx.fillStyle = C.t1; ctx.font = `700 11px var(--tf-font)`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText(formatBytes(storageData.total), cx, cy - 4);
    ctx.fillStyle = C.t3; ctx.font = `600 8px var(--tf-font)`; ctx.fillText('used', cx, cy + 7);
  }, [storageData]);

  const handleClearCache = useCallback(async () => {
    try { const cacheNames = await caches?.keys() || []; for (const name of cacheNames) await caches.delete(name); toast.success('Cache cleared'); fetchStorage(); }
  ...
  return (
    <Card className={st.cardPad}>
      ...
      <div className={st.subtitle}>{loading ? 'Calculating...' : `${formatBytes(storageData?.total || 0)} of ${formatBytes(storageData?.quota || 0)} (${usagePercent}%)`}</div>
          ...
          {usagePercent > 0 && (
            <div className={st.quotaWrap}>
              <div className={st.quotaTrack} style={{ background: C.bd + '20' }}>
                <div className={st.quotaFill} style={{ background: usagePercent > 80 ? C.r : C.b, width: `${Math.min(usagePercent, 100)}%` }} />
              </div>
              {usagePercent > 80 && <div className={st.quotaWarn}>⚠️ Storage usage is above 80%</div>}
            </div>
          )}

          <Btn variant="ghost" onClick={handleClearCache} style={{ fontSize: 11, padding: '6px 12px' }}>🗑 Clear Cache</Btn>
        </>
      )}
    </Card>
  );
