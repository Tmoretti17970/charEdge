...
function StatusBadge({ ok, label }) {
  return (
    <span className={st.statusBadge}
      style={{ background: ok ? `${C.g}12` : `${C.r}12`, color: ok ? C.g : C.r }}>
...
export default function PrivacyAIControls() {
  ...
  useEffect(() => {
    (async () => {
      try { const { conversationMemory } = await import('../../../ai/ConversationMemory'); setMemoryCount(conversationMemory.messageCount); } catch { /* */ }
    })();
  }, []);

  const handleClearMemory = useCallback(async () => {
    if (!confirm('Clear all AI conversation memory? This cannot be undone.')) return;
    try { const { conversationMemory } = await import('../../../ai/ConversationMemory'); await conversationMemory.reset(); setMemoryCount(0); } catch { /* */ }
  }, []);
  ...
  const handleClearPrefs = useCallback(async () => {
    ...
    try { const { adaptiveCoach } = await import('../../../ai/AdaptiveCoach'); adaptiveCoach.reset(); } catch { /* */ }
  }, []);

  return (
    <Card className={st.cardPad}>
      <div className={st.headerRow}><span>🧠</span> AI Privacy</div>
      ...
      <div className={st.shieldBanner} style={{ background: `${C.g}08`, border: `1px solid ${C.g}20` }}>
        ...
        <div>
          ...
          <div className={st.shieldHint}>Models run on-device. No data is sent to external servers unless you add a Cloud AI key.</div>
        </div>
      </div>
      ...
      <div className={st.clearRow}>
        <button onClick={handleClearMemory} className={`tf-btn ${st.clearBtn}`}
          style={{ border: `1px solid ${C.r}30`, background: `${C.r}06`, color: C.r }}>
          Clear AI Memory
        </button>
        <button onClick={handleClearPrefs} className={`tf-btn ${st.clearBtn}`}
