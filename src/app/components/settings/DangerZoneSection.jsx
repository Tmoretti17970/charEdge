...
function DangerAction({ title, description, buttonLabel, onClick, variant = 'danger', icon }) {
  return (
    <div className={st.actionRow}>
      ...
      <Btn variant={variant} onClick={onClick} style={{ fontSize: 11, padding: '6px 14px', flexShrink: 0 }}>
        {buttonLabel}
      </Btn>
    </div>
  );
...
function DangerZoneSection() {
  ...
  const handleClearAI = useCallback(() => {
    if (
      window.confirm(
        'Clear all AI learned data? This resets Trader DNA, conversation memory, and personalization. Your trades are preserved.',
      )
    ) {
      try {
        const aiKeys = Object.keys(localStorage).filter(
          (k) => k.includes('ai_') || k.includes('dna') || k.includes('memory') || k.includes('copilot'),
        );
