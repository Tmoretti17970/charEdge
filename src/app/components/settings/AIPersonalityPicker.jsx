    ...
    <Card className={st.cardPad}>
      ...
      <div className={st.grid}>
        {PERSONALITIES.map((p) => (
          <button
            ...
            className={`tf-btn ${st.personaBtn}`}
            style={{
          ...
          >
            <div className={st.personaHeader}>
              ...
              <span className={st.personaLabel} style={{ color: selected === p.id ? C.b : C.t1 }}>
                {p.label}
              </span>
            </div>
      ...
      <button
        onClick={() => {
          ...
          if (!showCustom) handleSelect('custom');
        }}
