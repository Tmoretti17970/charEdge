  ...
  return (
    <Card className={st.cardPad}>
      ...
      <div className={st.row}>
        {DENSITIES.map((d) => (
          <button key={d.id} onClick={() => handleChange(d.id)} className={`tf-btn ${st.densityBtn}`}
            style={{
            ...
            <div className={st.preview} style={{ gap: d.spacing / 2 }}>
              {Array.from({ length: d.rows }).map((_, i) => (
                <div key={i} className={st.previewLine}
                  style={{
            ...
            <div className={st.densityLabel} style={{ color: density === d.id ? C.b : C.t2 }}>{d.label}</div>
