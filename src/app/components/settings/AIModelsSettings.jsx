...
export default function AIModelsSettings() {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try { const avail = await mlPipeline.isAvailable(); if (!cancelled) setIsAvailable(avail); }
      catch { if (!cancelled) setIsAvailable(false); }
      try { const status = mlPipeline.getModelStatus(); if (!cancelled) setModelStatus(status); }
      catch { /* */ }
    })();
  ...
  const handlePreload = useCallback(async () => {
    try { await mlPipeline.preloadAll(); setModelStatus(mlPipeline.getModelStatus()); } catch { /* */ }
  }, []);
  ...
  return (
    <div className={st.root}>
      ...
      <div className={st.bulkRow}>
        <button onClick={enableAll} disabled={!mlEnabled} className={st.bulkBtn}
          style={{ color: mlEnabled ? C.t2 : C.t3, cursor: mlEnabled ? 'pointer' : 'not-allowed', opacity: mlEnabled ? 1 : 0.5 }}>
          Enable All
        </button>
        <button onClick={disableAll} disabled={!mlEnabled} className={st.bulkBtn}
          style={{ color: mlEnabled ? C.t2 : C.t3, cursor: mlEnabled ? 'pointer' : 'not-allowed', opacity: mlEnabled ? 1 : 0.5 }}>
          Disable All
        </button>
        <button onClick={handlePreload} disabled={!mlEnabled || !isAvailable}
          className={st.preloadBtn}
          style={{ cursor: mlEnabled && isAvailable ? 'pointer' : 'not-allowed', opacity: mlEnabled && isAvailable ? 1 : 0.5 }}>
          Preload All
        </button>
      </div>
      ...
      <div className={st.modelList}>
        {modelIds.map((id) => {
          ...
          return (
            <div key={id} id={`model-setting-${id}`} className={st.modelRow}
              style={{
              ...
              <div className={st.modelBody}>
                ...
                <div className={st.modelStats}>
                  {status && <span style={{ color: status.loaded ? C.g : C.t3 }}>{status.loaded ? '● Loaded' : '○ Unloaded'}</span>}
                  {status?.sizeKB && <span style={{ color: C.t3 }}>{status.sizeKB}KB</span>}
                  {stats && <span style={{ color: '#6e5ce6' }}>{stats.count} runs · {Math.round(stats.lastMs)}ms last</span>}
                </div>
              </div>
...
function Toggle({ checked, onChange, label, disabled = false }) {
  return (
    <button role="switch" aria-checked={checked} aria-label={label || 'Toggle'}
      disabled={disabled} onClick={onChange} className={st.toggleTrack}
      style={{ background: checked ? C.g : C.bd, cursor: disabled ? 'not-allowed' : 'pointer', opacity: disabled ? 0.4 : 1 }}>
      <div className={st.toggleThumb}
