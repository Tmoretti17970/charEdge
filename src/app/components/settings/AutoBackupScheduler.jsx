...
function AutoBackupScheduler() {
  ...
  const handleFrequency = useCallback((id) => { setFrequency(id); if (typeof updateSetting === 'function') updateSetting({ backupFrequency: id }); }, [updateSetting]);
  const handleRetention = useCallback((val) => { setRetention(val); if (typeof updateSetting === 'function') updateSetting({ backupRetention: val }); }, [updateSetting]);
  ...
  return (
    <Card className={st.cardPad}>
      <div className={st.header} style={{ marginBottom: enabled ? 14 : 0 }}>
        ...
        <button onClick={toggleBackup} className={`tf-btn ${st.toggleTrack}`}
        ...
        <>
          <div className={st.optionBlock}>
            ...
            <div className={st.optionRow}>
              {FREQUENCIES.map((f) => (
                <button key={f.id} onClick={() => handleFrequency(f.id)} className={`tf-btn ${st.optionBtn}`}
                  style={{ border: `1px solid ${frequency === f.id ? C.b : C.bd + '30'}`, background: frequency === f.id ? C.b + '08' : 'transparent', color: frequency === f.id ? C.b : C.t2 }}>
                  {f.label}
                </button>
              ))}
            </div>
          ...
          <div className={st.optionBlock} style={{ marginBottom: 14 }}>
            ...
            <div className={st.optionRow}>
              {RETENTIONS.map((r) => (
                <button key={r.value} onClick={() => handleRetention(r.value)} className={`tf-btn ${st.optionBtn}`}
                  style={{ border: `1px solid ${retention === r.value ? C.b : C.bd + '30'}`, background: retention === r.value ? C.b + '08' : 'transparent', color: retention === r.value ? C.b : C.t2 }}>
                  {r.label}
                </button>
              ))}
            </div>
          </div>
          <Btn variant="ghost" onClick={handleBackupNow} style={{ fontSize: 11, padding: '6px 14px' }}>📥 Backup Now</Btn>
        </>
      )}
    </Card>
  );
