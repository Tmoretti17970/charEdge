...
function detectCandlestickPatterns(candles) {
  ...
  if (
    candles.length >= 3 &&
    !candles[last - 2].close > candles[last - 2].open &&
...
function detectChartPatterns(candles) {
  ...
  if (swingHighs.length >= 2 && swingLows.length >= 2) {
    ...
    const highsFalling =
      recentHighs.length >= 2 && recentHighs[recentHighs.length - 1].price < recentHighs[0].price * 0.995;
...
function interpretVolume(features) {
  ...
  if (volumeSpike > 3)
    insights.push(`🚨 Massive volume spike (${volumeSpike.toFixed(1)}x) — institutional activity likely`);
  else if (volumeSpike > 2)
    insights.push(`⚡ Significant volume surge (${volumeSpike.toFixed(1)}x) — conviction behind this move`);
  else if (volumeSpike > 1.5)
    insights.push(`📊 Above-average volume (${volumeSpike.toFixed(1)}x) — interested participants`);
  ...
  if (buyPressure > 0.75)
    insights.push(`🟢 Strong buy pressure (${(buyPressure * 100).toFixed(0)}%) — buyers in control`);
  else if (buyPressure > 0.6) insights.push(`📈 Healthy buy pressure (${(buyPressure * 100).toFixed(0)}%)`);
  else if (buyPressure < 0.25)
    insights.push(`🔴 Heavy sell pressure (${((1 - buyPressure) * 100).toFixed(0)}%) — sellers dominating`);
  else if (buyPressure < 0.4)
    insights.push(`📉 Bears have the edge (${((1 - buyPressure) * 100).toFixed(0)}% sell pressure)`);
...
function assessRisk(features, regime) {
  ...
  if (momentum.rsi > 80 || momentum.rsi < 20) {
    riskScore += 25;
    risks.push('Extreme RSI — reversal risk elevated');
  } else if (momentum.rsi > 70 || momentum.rsi < 30) {
    riskScore += 15;
    risks.push('RSI approaching extremes');
  }

  // Volatility risk
  if (volatility.atrRatio > 0.04) {
    riskScore += 20;
    risks.push('High volatility — widen stops or reduce size');
  }
  if (volatility.bollingerWidth > 0.08) {
    riskScore += 10;
    risks.push('Bollinger Bands very wide — chaotic price action');
  }
  if (volatility.bollingerWidth < 0.01) {
    riskScore += 5;
    risks.push('Extreme squeeze — explosive move imminent, direction uncertain');
  }

  // Volume risk
  if (volume.volumeRatio < 0.5) {
    riskScore += 15;
    risks.push('Low volume — breakouts will fake out');
  }
  if (volume.volumeSpike > 3) {
    riskScore += 10;
    risks.push('Volume spike — potential climax/exhaustion');
  }

  // Regime risk
  if (regime === REGIMES.CHOPPY) {
    riskScore += 20;
    risks.push('Choppy regime — entries get stopped out repeatedly');
  }
  if (regime === REGIMES.REVERSAL) {
    riskScore += 15;
    risks.push('Reversal zone — wrong-side risk is high');
  }
...
function generateKeyObservations(features, regime, divs, patterns, risk, _candles) {
  const obs = [];
...
function detectKeyLevels(candles, lookback = 50) {
  ...
  for (let i = 2; i < window.length - 2; i++) {
    ...
    const isSwingHigh =
      ...
      c.high > window[i + 2].high;
    const isSwingLow =
      c.low < window[i - 1].low && c.low < window[i - 2].low && c.low < window[i + 1].low && c.low < window[i + 2].low;
...
function generatePulseText(regime, features, symbol, _tf) {
  ...
  const volDesc =
    volume.volumeRatio > 1.5 ? 'elevated volume' : volume.volumeRatio < 0.5 ? 'muted volume' : 'normal volume';
  const rsiDesc =
    momentum.rsi > 70
      ? 'overbought'
      : momentum.rsi < 30
        ...
        : momentum.rsi > 55
          ...
          : momentum.rsi < 45
            ? 'mildly bearish'
            : 'neutral';
  const trendWord = momentum.trendStrength > 25 ? 'powerfully' : momentum.trendStrength > 15 ? 'steadily' : 'slowly';
  const emaContext =
    Math.abs(momentum.priceVsEma) > 0.02
...
export const localInsightEngine = {
  ...
  generateFullAnalysis(features, symbol, tf, candles) {
    ...
    const sections = [
      ...
      },
      {
        ...
        detail:
          volatility.bollingerWidth < 0.015
            ? '🔋 Squeeze forming — expect explosive expansion'
            : volatility.bollingerWidth < 0.02
              ? 'Squeeze forming — expect expansion'
              : volatility.bollingerWidth > 0.08
                ? '🔥 Extreme range — manage risk aggressively, widen stops'
                : volatility.bollingerWidth > 0.06
                  ? 'Wide range — manage risk tightly'
    ...
    if (levels.length > 0) {
      sections.push({
        ...
        content: levels
          .slice(0, 3)
          .map((l) => `${l.type === 'support' ? '🟢' : '🔴'} ${l.price} (${l.distance > 0 ? '+' : ''}${l.distance}%)`)
          .join(' · '),
  ...
  generateDetailedNarrative(features, symbol, tf, candles) {
    ...
    if (levels.length > 0) {
      ...
      paras.push(
        `Nearest ${nearest.type} at $${nearest.price} (${Math.abs(parseFloat(nearest.distance))}% ${dir} current price). ${levels.length > 1 ? `${levels.length - 1} more levels nearby.` : ''}`,
      );
    }

    // P5: Risk + bias
    paras.push(
      `${risk.emoji} Risk: ${risk.level} (${risk.score}/100). Setup: ${grade.letter} (${score}/100). ${generateBias(regime, features, lastClose)}`,
    );
