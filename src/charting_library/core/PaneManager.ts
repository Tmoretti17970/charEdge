...
export class PaneManager {
  ...
  constructor(engineContainer: HTMLElement, options?: { onResize?: () => void }) {
    ...
    this._flexContainer.style.cssText = 'display:flex;flex-direction:column;width:100%;height:100%;overflow:hidden;';
  ...
  syncIndicators(
    ...
    while (this._indicatorPanes.length < desiredCount) {
      ...
      this._addIndicatorPane(paneInds[idx]!, idx, paneHeights?.[idx], collapsedPanes?.has(idx));
    }
  ...
  private _addIndicatorPane(
    ...
    container.style.cssText = `flex:0 0 ${MIN_PANE_PX}px;position:relative;overflow:hidden;min-height:0;`;
  ...
  private _createSplitter(paneIdx: number): { splitter: HTMLDivElement; ac: AbortController } {
    ...
    grip.style.cssText = ['display:flex', 'gap:4px', 'pointer-events:none'].join(';') + ';';
    ...
    splitter.addEventListener(
      ...
      () => {
        ...
        dots.forEach((d: unknown) => {
          d.style.background = '#2962FF';
        });
      },
      { signal },
    );
    splitter.addEventListener(
      ...
      () => {
        if (!this._dragState.active) {
          ...
          const dots = grip.querySelectorAll('div');
          dots.forEach((d: unknown) => {
            d.style.background = 'rgba(120,123,134,0.6)';
          });
        }
      },
      { signal },
  ...
  private _onSplitterDragEnd(): void {
    ...
    for (const pane of this._indicatorPanes) {
      ...
      // Clear any residual transition to avoid delayed layout shifts
  ...
  private _relayout(): void {
    ...
    for (const pane of this._indicatorPanes) {
      ...
      // NOTE: CSS transition removed from _relayout() — it caused
      // ResizeObserver to fire many intermediate size changes during
      // splitter drag, each clearing the canvas buffer (setting
      // canvas.width inherently clears content), producing a
      // black/white flash. Transitions are now only applied
      // explicitly in toggleCollapse() where animation is desired.
    }
