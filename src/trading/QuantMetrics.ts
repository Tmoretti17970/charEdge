...
export function stdDev(values: number[]): number {
    ...
    // Sample standard deviation (n-1) — correct for Sharpe/Sortino on finite trade samples
...
export function downsideDev(
    ...
    const downsideSquared = values
        ...
        .map((v) => v ** 2);
    // Sample downside deviation (n-1) — matches sample stdDev convention
    return Math.sqrt(downsideSquared.reduce((sum, v) => sum + v, 0) / (values.length - 1));
}
...
export function sharpeRatio(
    ...
    // Cap at ±99.99 instead of Infinity to prevent UI/chart/CSV corruption
    if (sigma === 0) return avgExcess >= 0 ? 99.99 : -99.99;
...
export function sortinoRatio(
    ...
    // Cap at ±99.99 instead of Infinity to prevent UI/chart/CSV corruption
    if (dsigma === 0) return avgExcess >= 0 ? 99.99 : -99.99;
...
export function computeQuantSummary(
    ...
    // Max drawdown — track equity (startingEquity + cumPnL), not raw P&L
    let peakEquity = startingEquity;
    let maxDD = 0;
    for (const c of cumulative) {
        const equity = startingEquity + c;
