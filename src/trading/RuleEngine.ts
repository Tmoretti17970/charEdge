...
export function evaluateAllTrades(
    ...
    // Pre-sort once — avoids O(n² log n) from re-sorting inside every rule check
...
function checkMaxDailyTrades(
    ...
    const limit = Number(rule.params.limit);
    if (isNaN(limit)) return null; // Misconfigured rule — skip silently
...
function checkMaxConsecutiveLosses(
    ...
    // allTrades is already pre-sorted chronologically by evaluateAllTrades
    let streak = 0;
