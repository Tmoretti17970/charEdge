...
function classifyCondition(momentum: string, volatility: string): string {
  if (volatility.includes('High') && momentum.includes('Overbought')) return '⚠️ Potential Reversal';
  if (volatility.includes('Low') && momentum.includes('Neutral')) return '💤 Consolidation';
  if (momentum.includes('Strong Up')) return '🚀 Trending Up';
  if (momentum.includes('Strong Down')) return '🔻 Trending Down';
    ...
    const unsub = useChartCoreStore.subscribe((state: unknown) => {
      ...
      debounceRef.current = setTimeout(() => {
        try {
          ...
          const momentumLabel = classifyMomentum(features.momentum.rsi, features.momentum.trendStrength);
          const volatilityLabel = classifyVolatility(features.volatility.atrRatio, features.volatility.bollingerWidth);
          const volumeLabel = classifyVolume(features.volume.volumeRatio, features.volume.volumeSpike);
...
Give a 2-3 sentence market narrative covering current conditions, key levels to watch, and what to expect next.`;
        ...
        /* AIRouter not available, fall through */
      }
  ...
  const requestPatterns = useCallback(async () => {
    const state = useChartCoreStore.getState() as Record<string, unknown>;
    const bars = (state.data || []) as unknown[];
