...
const TRADE_PATTERNS = [
  /\bmy\s+trade/i, /\bbest\s+trade/i, /\bworst\s+trade/i,
  /\blast\s+trade/i, /\brecent\s+trade/i, /\bfirst\s+trade/i,
  /\btrades?\s+(this|last|on|from|in)\b/i,
  /\bwin\s*rate/i, /\blose?\s*rate/i, /\bstrike\s*rate/i,
  /\btrading\s+(history|record|log|stats|performance)/i,
  /\bjournal/i, /\blogbook/i,
  ...
  /\bovertrading/i, /\brevenge\s+trad/i,
...
const useCopilotChat = create<CopilotChatState>((set, get) => ({
  ...
  recordFeedback(msgId: string, direction: 'positive' | 'negative') {
    ...
    import('@/ai/ConversationLearning').then(({ conversationLearning }) => {
  ...
  async loadHistory() {
    ...
    try {
      ...
      if (session && session.messages.length > 0) {
        const restored: CopilotMessage[] = session.messages.map((m: Record<string, unknown>, i: number) => ({
          ...
          tier: ((m.metadata as Record<string, unknown>)?.tier as string) || (m.role === 'assistant' ? 'L3' : undefined),
    ...
    import('@/ai/WebLLMProvider').then(({ webLLMProvider }) => {
      webLLMProvider.preloadIfOptedIn();
    }).catch(() => {});

    // Sprint 33: Subscribe to proactive insights
    import('@/ai/ProactiveInsightManager').then(({ proactiveInsightManager }) => {
    ...
    if (!localStorage.getItem(briefKey)) {
      import('@/ai/AIBriefService').then(async ({ aiBriefService }) => {
        try {
  ...
  async sendMessage(text: string) {
    ...
    import('@/ai/ConversationMemory').then(({ conversationMemory }) => {
    ...
    try {
      ...
      import('@/ai/ConversationMemory').then(({ conversationMemory }) => {
        conversationMemory.addMessage('assistant', fullText, {
          tier: 'L3',
          journalContext: usedJournalContext,
        }).catch(() => {});
      }).catch(() => {});
