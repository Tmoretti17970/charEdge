...
describe('Phase 1.1.1 — Render-on-demand (kill continuous rAF)', () => {
  ...
  beforeEach(() => {
    engineSource = fs.readFileSync(
      path.resolve(__dirname, '..', 'charting_library/core/ChartEngine.js'),
      'utf-8'
    );
  });
  ...
  it('markDirty() calls _scheduleDraw()', () => {
    const markDirtySection = engineSource.match(/markDirty\(\)\s*\{[\s\S]*?\n\s*\}/)?.[0] || '';
  ...
  it('setIndicators() calls _scheduleDraw()', () => {
    const section = engineSource.match(/setIndicators\(indicators\)\s*\{[\s\S]*?\n\s*\}/)?.[0] || '';
    expect(section).toContain('this._scheduleDraw()');
  });

  it('setAlerts() calls _scheduleDraw()', () => {
    const section = engineSource.match(/setAlerts\(alerts\)\s*\{[\s\S]*?\n\s*\}/)?.[0] || '';
    expect(section).toContain('this._scheduleDraw()');
  });

  it('setSyncedCrosshair() calls _scheduleDraw()', () => {
    const section = engineSource.match(/setSyncedCrosshair\(data\)\s*\{[\s\S]*?\n\s*\}/)?.[0] || '';
    expect(section).toContain('this._scheduleDraw()');
  });
...
describe('Phase 1.1.2 — Incremental bar append (blit-pan)', () => {
  ...
  beforeEach(() => {
    frameStateSource = fs.readFileSync(
      path.resolve(__dirname, '..', 'charting_library/core/FrameState.js'),
      'utf-8'
    );
    dataStageSource = fs.readFileSync(
      path.resolve(__dirname, '..', 'charting_library/core/stages/DataStage.js'),
      'utf-8'
    );
    pipelineSource = fs.readFileSync(
      path.resolve(__dirname, '..', 'charting_library/core/RenderPipeline.js'),
      'utf-8'
    );
    engineSource = fs.readFileSync(
      path.resolve(__dirname, '..', 'charting_library/core/ChartEngine.js'),
  ...
  it('DataStage imports CHANGED from FrameState', () => {
    expect(dataStageSource).toContain("import { CHANGED } from '../FrameState.js'");
  });
...
describe('Phase 1.1.3 — Pre-rasterize static layers (grid/axes cache)', () => {
  ...
  beforeEach(() => {
    gridSource = fs.readFileSync(
      path.resolve(__dirname, '..', 'charting_library/core/stages/GridStage.js'),
      'utf-8'
    );
    axesSource = fs.readFileSync(
      path.resolve(__dirname, '..', 'charting_library/core/stages/AxesStage.js'),
      'utf-8'
    );
  });
...
describe('Phase 1.1.4 — Virtual bar window (GPU buffer management)', () => {
  ...
  beforeEach(() => {
    rendererSource = fs.readFileSync(
      path.resolve(__dirname, '..', 'charting_library/renderers/WebGLRenderer.js'),
      'utf-8'
    );
  });
