...
describe('TriggerCorrelation', () => {
    ...
    it('detects sabotage clusters', () => {
        const now = Date.now();
        // 3 losses on same day-of-week and time
        const trades = [
            makeTrade({ pnl: -100, triggers: ['fatigue'], date: new Date(now - 7 * 86400000).toISOString() }),
            makeTrade({ pnl: -80, triggers: ['fatigue'], date: new Date(now - 7 * 86400000 + 3600000).toISOString() }),
            makeTrade({ pnl: -60, triggers: ['fatigue'], date: new Date(now - 7 * 86400000 + 7200000).toISOString() }),
        ];
        const report = analyzeTriggerCorrelation(trades);
