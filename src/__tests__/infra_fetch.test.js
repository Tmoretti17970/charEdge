...
describe('FetchService — delta-only fetching support', () => {
  it('_fetchBinanceBatch accepts startTime parameter', async () => {
    const source = readSource('data/FetchService.js');
    expect(source).toContain('async function _fetchBinanceBatch(pair, interval, limit, endTime, startTime)');
...
describe('FetchService — config maps have no duplicates', () => {
  ...
  it('TTL map has no duplicate keys', () => { expect(findDuplicateKeys(source, 'const TTL')).toEqual([]); });
  it('BINANCE_INTERVALS map has no duplicate keys', () => { expect(findDuplicateKeys(source, 'const BINANCE_INTERVALS')).toEqual([]); });
  it('BINANCE_LIMITS map has no duplicate keys', () => { expect(findDuplicateKeys(source, 'const BINANCE_LIMITS')).toEqual([]); });
  it('BINANCE_PAGINATE_PAGES map has no duplicate keys', () => { expect(findDuplicateKeys(source, 'const BINANCE_PAGINATE_PAGES')).toEqual([]); });
  it('ADJACENT map has no duplicate keys', () => { expect(findDuplicateKeys(source, 'const ADJACENT')).toEqual([]); });
  it('CG_TF_MAP has no duplicate keys', () => { expect(findDuplicateKeys(source, 'const CG_TF_MAP')).toEqual([]); });
  it('CC_TF_MAP has no duplicate keys', () => { expect(findDuplicateKeys(source, 'const CC_TF_MAP')).toEqual([]); });
  it('YAHOO_TF_MAP has no duplicate keys', () => { expect(findDuplicateKeys(source, 'const YAHOO_TF_MAP')).toEqual([]); });

  it('TTL covers 30m and 1w', () => {
    const block = source.slice(source.indexOf('const TTL'), source.indexOf('const TTL') + 300);
    expect(block).toContain("'30m'");
    expect(block).toContain("'1w'");
  });

  it('BINANCE_INTERVALS covers 30m and 1w', () => {
    const block = source.slice(source.indexOf('const BINANCE_INTERVALS'), source.indexOf('const BINANCE_INTERVALS') + 300);
    expect(block).toContain("'30m'");
    expect(block).toContain("'1w'");
  });

  it('has no range-based legacy keys', () => {
    const ttlIdx = source.indexOf('const TTL');
    const ttlEnd = source.indexOf('};', ttlIdx) + 2;
    const ttlBlock = source.slice(ttlIdx, ttlEnd);
...
describe('isCrypto unification', () => {
  ...
  it('WebSocketService imports isCrypto from constants.js', () => {
