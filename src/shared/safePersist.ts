...
//
// Usage:
//   import { safePersist } from './safePersist';
//
  ...
  return persist(storeCreator, {
    ...
    storage: customStorage || {
      getItem: (storageKey) => {
        try {
          ...
          if (!parsed || typeof parsed !== 'object') {
            logger.ui.warn(`${TAG} Invalid state structure for "${name}", resetting.`);
          ...
          logger.ui.warn(`${TAG} Failed to read "${name}" from storage:`, err.message);
      ...
      setItem: (storageKey, value) => {
        try {
          ...
          if (err.name === 'QuotaExceededError' || err.code === 22) {
            logger.ui.warn(`${TAG} Storage quota exceeded for "${name}". Data will not persist.`);
          } else {
            logger.ui.warn(`${TAG} Failed to write "${name}":`, err.message);
          }
      ...
      removeItem: (storageKey) => {
        try {
          ...
          logger.ui.warn(`${TAG} Failed to remove "${name}":`, err.message);
        }
      },
    },
    ...
    merge: (persistedState, currentState) => {
      try {
        ...
        logger.ui.warn(`${TAG} Merge failed for "${name}", using defaults.`);
      ...
      ? {
          migrate: (persisted, persistedVersion) => {
            try {
              ...
              logger.ui.warn(`${TAG} Migration failed for "${name}" (v${persistedVersion} → v${version}):`, err.message);
