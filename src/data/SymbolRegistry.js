...
class _SymbolRegistry {
  ...
  search(query, limit = 20) {
    ...
    // Check aliases first for exact match redirect
    const aliased = this._aliases.get(q);
    if (aliased && this._map.has(aliased)) {
      scored.push({ info: this._map.get(aliased), score: 100 });
    }

    for (const [sym, info] of this._map) {
      if (aliased && sym === aliased) continue; // Already added via alias

      let score = 0;
      const name = (info.displayName || '').toUpperCase();
      ...
      // Symbol prefix
        ...
        score = 80 + (q.length / sym.length) * 15;
      }
      // Display name prefix
      else if (name.startsWith(q)) {
        score = 70 + (q.length / name.length) * 10;
      }
      // Symbol contains
      ...
      // Fuzzy: all query chars exist in order in symbol or name
      else if (_fuzzyMatch(q, sym) || _fuzzyMatch(q, name)) {
        score = 20;
      }

      if (score > 0) {
        // Boost real-time symbols slightly
        if (info.realtime) score += 2;
  ...
  searchByClass(assetClass, limit = 50) {
    return this.all()
