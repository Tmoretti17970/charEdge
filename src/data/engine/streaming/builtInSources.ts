...
const _adapterCache = new Map<string, Promise<Record<string, unknown>>>();
function lazy(path: string): Promise<Record<string, unknown>> {
...
// Typed data helper to access fields on unknown objects
...
export function registerBuiltInSources(sources: Map<string, SourceAdapter>): void {
  ...
  sources.set('pyth', {
    ...
    fetchQuote: async (symbol: string) => {
      const { pythAdapter } = await lazy('../../adapters/PythAdapter.js');
      ...
      try {
        ...
        const base =
          typeof window === 'undefined'
            ? `http://localhost:${(globalThis as unknown as Record<string, unknown>).__TF_PORT || 3000}`
            : '';
  ...
  sources.set('finnhub', {
    ...
    subscribe: (symbol: string, callback: PriceCallback) => {
      let unsub: (() => void) | null = null;
      let disposed = false;
      lazy('../../adapters/FinnhubAdapter.js')
        .then((m) => {
    ...
    fetchQuote: async (symbol: string) => {
      const { finnhubAdapter } = await lazy('../../adapters/FinnhubAdapter.js');
  ...
  sources.set('forex', {
    ...
    fetchQuote: async (symbol: string) => {
      const { forexAdapter } = await lazy('../../adapters/ForexAdapter.js');
  ...
  sources.set('kraken', {
    ...
    subscribe: (symbol: string, callback: PriceCallback) => {
      let unsub: (() => void) | null = null;
      let disposed = false;
      lazy('../../adapters/KrakenAdapter.js')
        .then((m) => {
    ...
    fetchQuote: async (symbol: string) => {
      const { krakenAdapter } = await lazy('../../adapters/KrakenAdapter.js');
  ...
  sources.set('bybit', {
    ...
    fetchQuote: async (symbol: string) => {
      const { bybitAdapter } = await lazy('../../adapters/BybitAdapter.js');
  ...
  sources.set('okx', {
    ...
    fetchQuote: async (symbol: string) => {
      const { okxAdapter } = await lazy('../../adapters/OKXAdapter.js');
  ...
  sources.set('coinbase', {
    ...
    subscribe: (symbol: string, callback: PriceCallback) => {
      let unsub: (() => void) | null = null;
      let disposed = false;
      lazy('../../adapters/CoinbaseAdapter.js')
        .then((m) => {
    ...
    fetchQuote: async (symbol: string) => {
      const { coinbaseAdapter } = await lazy('../../adapters/CoinbaseAdapter.js');
  ...
  sources.set('coincap', {
    ...
    subscribe: (symbol: string, callback: PriceCallback) => {
      let unsub: (() => void) | null = null;
      let disposed = false;
      import('../../adapters/CoinCapAdapter.js')
        .then(({ coinCapAdapter }) => {
          ...
          unsub = coinCapAdapter.subscribe(symbol, (data: unknown) => {
            ...
            callback({
              ...
              timestamp: (d.time as number) || Date.now(),
              confidence: 0,
        ...
        .catch(() => {
          /* CoinCapAdapter import failed — skip */
        });
      return () => {
        disposed = true;
        if (typeof unsub === 'function') unsub();
