...
function flushPushGroup(): void {
  ...
  if (_pushGroupBuffer.length === 1) {
    ...
    const catLabel = count > 1 ? `${count} new notifications` : _pushGroupBuffer[0].title;
    ...
    try {
      ...
      logger.data.warn('[NotificationRouter] Push group delivery failed:', (err as Error)?.message);
    }
  }
...
function updateBadge(): void {
  try {
    ...
    logger.data.warn('[NotificationRouter] Badge update failed:', (err as Error)?.message);
  }
}
...
export function clearBadge(): void {
  ...
  try {
    ...
    logger.data.warn('[NotificationRouter] Badge clear failed:', (err as Error)?.message);
  }
}
...
const CHANNEL_HANDLERS: Record<ChannelKey, (p: NotificationPayload) => void> = {
  ...
  email: (p) => {
    addToDigest(p);
  }, // Sprint 14: buffer for email digest
  sound: deliverSound,
};
