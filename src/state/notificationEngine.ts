...
const CATEGORY_ACCENT: Record<NotificationCategoryId, string> = {
  securityAlerts: '#f59e0b',
  priceAlerts: '#22c55e',
  customAlerts: '#3b82f6',
  tradingInsights: '#6366f1',
  advancedTransactions: '#22c55e',
  offersAnnouncements: '#8b5cf6',
  smartAlerts: '#3b82f6',
  system: '#6b7280',
};
...
function flushPushGroup(): void {
  ...
  if (_pushGroupBuffer.length === 1) {
    deliverSinglePush(_pushGroupBuffer[0]);
  } else {
    ...
    try {
      ...
      logger.data.warn('[NotificationEngine] Push group failed:', (err as Error)?.message);
    }
  }
...
function deliverSinglePush(payload: NotificationPayload): void {
  ...
  try {
    ...
    logger.data.warn('[NotificationEngine] Push failed:', (err as Error)?.message);
  }
}
...
function updateBadge(): void {
  try {
    if ('setAppBadge' in navigator)
      (navigator as Navigator & { setAppBadge: (count: number) => Promise<void> }).setAppBadge(_pushGroupCount);
  } catch (err) {
    logger.data.warn('[NotificationEngine] Badge update failed:', (err as Error)?.message);
  }
}
...
export function clearBadge(): void {
  _pushGroupCount = 0;
  try {
    if ('clearAppBadge' in navigator) (navigator as Navigator & { clearAppBadge: () => Promise<void> }).clearAppBadge();
  } catch (err) {
    logger.data.warn('[NotificationEngine] Badge clear failed:', (err as Error)?.message);
  }
}
...
function deliverSound(payload: NotificationPayload): void {
  ...
  try {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- soundType union mismatch with non-exported SoundType
    playAlertSound(soundType as any, getAlertVolume());
  } catch {
    /* audio may be blocked by autoplay policy */
  }
}
...
export function getAlertAnalytics(alerts: AlertSnapshot[], history: HistoryEntry[]): AlertAnalytics {
  ...
  symbolCondMap.forEach((count, key) => {
    if (count > maxCount) {
      maxCount = count;
      const d = symbolCondDetails.get(key);
      mostActive = { symbol: d?.symbol || key, condition: d?.condition || '', count };
    }
  });
  ...
  if (staleAlerts.length > 3)
    suggestions.push({
      ...
      body: `${staleAlerts.length} alerts haven't triggered in ${STALE_DAYS}+ days.`,
      icon: '🧹',
  ...
  for (const [symbol, count] of Object.entries(alertsBySymbol).filter(([, c]) => c >= 5)) {
    suggestions.push({
      ...
      body: `You have ${count} alerts on ${symbol}. Consider using a template.`,
      icon: '📦',
  ...
  if (falsePositiveRate > 0.5 && withAfterPrice.length >= 5)
    suggestions.push({
      ...
      body: `${Math.round(falsePositiveRate * 100)}% of alerts reversed quickly. Try "Balanced" mode.`,
      icon: '📊',
      relatedAlertIds: [],
    });
