// ═══════════════════════════════════════════════════════════════════
// charEdge — Vercel Serverless Proxy: Tiingo
//
...
export default async function handler(req, res) {
    ...
    if (!apiToken) {
        return res.status(503).json({ ok: false, error: 'Tiingo API token not configured' });
    }
    ...
    try {
        ...
        res.status(502).json({ ok: false, error: `Tiingo proxy error: ${err.message}` });
    }
}
