// ═══════════════════════════════════════════════════════════════════
// charEdge — Vercel Serverless Proxy: FRED (Federal Reserve)
//
...
export default async function handler(req, res) {
  ...
  if (!apiKey) {
    return res.status(503).json({ ok: false, error: 'FRED API key not configured' });
  }
  ...
  try {
    ...
    res.setHeader('Cache-Control', 'public, max-age=300'); // 5 min for economic data
    ...
    res.status(502).json({ ok: false, error: `FRED proxy error: ${err.message}` });
  }
}
