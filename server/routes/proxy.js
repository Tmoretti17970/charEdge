...
export function createProxyRouter() {
  ...
  router.get('/api/proxy/:provider/*', async (req, res) => {
    ...
    if (isHeaderAuth && config.envKeys && config.headerMap) {
      for (const [key, envName] of Object.entries(config.envKeys)) {
        ...
        if (!val) {
          return res
            .status(503)
            .json({ ok: false, error: `${provider.toUpperCase()} API key not configured (missing ${envName})` });
        }
    ...
    const symbolParam = /** @type {string|undefined} */ (
      req.query.symbol || req.query.ticker || req.query.tickers || req.query.q
    );
    ...
    // SSRF defense-in-depth: ensure constructed URL still targets the expected domain
    const url = new URL(`${config.base}/${proxyPath}`);
    const expectedHost = new URL(config.base).hostname;
    if (url.hostname !== expectedHost) {
      return res.status(400).json({ ok: false, error: 'URL host mismatch — possible SSRF attempt' });
    }
